...
class MujocoEnv(BaseMujocoEnv):
    ...
    def __init__(
        ...
        act_dim = self.action_space.shape[0]+extra
        act_limit = self.action_space.high[0]
        self.action_space = gym.spaces.Box(low=np.array([-act_limit for i in range(act_dim )]), high=np.array([act_limit for i in range(act_dim)]), dtype=np.float32)
